...
const task = body.querySelector('#coinn');
...
image.addEventListener('click' , (e)=> {
    ...
    if(Number(power) > 0){
        localStorage.setItem('coins' , `${Number(coins) + 1.13}`);
...
task.addEventListener('click' , (e)=> {
    ...
    if(Number(power) > 0){
        localStorage.setItem('coins' , `${Number(coins) + 200.13}`);
        h1.textContent = `${(Number(coins) + 200).toLocaleString()}`;
